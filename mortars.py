...
def credit(Theta,Vi,BLDG1,BLDG2) :
    ...
    bestTheta1,bestTheta2 = Solver(BLDG1, BLDG2, Vi, Theta)
...
def Solver(pointA, pointB, Vi, Theta):
...
VelocityX = Vi*cos(ThetaRad)
VelocityY = Vi*sin(ThetaRad)
...
wVi = 53                 # m/s (Initial Velocity)        
wTheta = 77.5
wThetaRad = radians(wTheta)     # degrees (Launch Angle Theta)
wVelocityX = wVi*cos(wThetaRad)
wVelocityY = wVi*sin(wThetaRad)
# Red Mortar
wDirection1 = radians(12)  # Adjustment for aiming in degrees then rads
# Green Mortar
wDirection2 = radians(200) # Adjustment for aiming in degrees
...
credit(Theta,Vi,bldg1a.pos,bldg2a.pos)
...
PresentWith = 'All'        # Select to show Wind or not, All, Wind, NoWind
...
if PresentWith == 'NoWind' :
    ...
    while (W1ball.pos.y and W2ball.pos.y) > (floorPosY + 1) :
        if scene.mouse.clicked:             # Control play with mouse click
            scene.mouse.getclick()
            scene.mouse.getclick() 
        rate(speed/dt)     
        ...
        W1ball.acceleration = g*m -(D/m)*mag(W1ball.velocity)*W1ball.velocity + Wind
